...
function initCategory(data, base_url) {
    baseURL = base_url;
    recipes = [...data];
    if (recipes.length === 0) {
        document.querySelector('.recipe_message_error').innerHTML = `<p class="h2 text-center my-10">No se encontraron recetas, con la categoría seleccionada.</p>`;
    } else {
        render();
        watchMarker();
    }
